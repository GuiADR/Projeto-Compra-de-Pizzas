...
pizzaJson.map((item, index)=>{
  ...
  pizzaItem.querySelector('.pizza-item--price').innerHTML = `R$ ${item.sizes[2].price.toFixed(2)}`;
  ...
  pizzaItem.querySelector('a').addEventListener('click',(e)=>{
    ...
    q('.pizzaWindowArea').setAttribute('data-key', key);
    ...
    q('.pizzaInfo--actualPrice').innerHTML = `R$ ${pizzaJson[key].sizes[2].price.toFixed(2)}`;
    q('.pizzaInfo--size.selected').classList.remove('selected');
    qa('.pizzaInfo--size').forEach((size, sizeIndex) =>{
      if(sizeIndex == 2){
        size.classList.add('selected');
      }
      size.querySelector('span').innerHTML = pizzaJson[key].sizes[sizeIndex].weight;
    });
...
qa('.pizzaInfo--cancelButton, pizzaInfo--MobileButton').forEach((item)=>{
...
qa('.pizzaInfo--size').forEach((size, sizeIndex) =>{
  size.addEventListener('click', ()=>{
    q('.pizzaInfo--size.selected').classList.remove('selected');
    size.classList.add('selected');
    let key = q('.pizzaWindowArea').getAttribute('data-key');
    q('.pizzaInfo--actualPrice').innerHTML = `R$ ${pizzaJson[key].sizes[sizeIndex].price.toFixed(2)}`;
  });
});
q('.pizzaInfo--addButton').addEventListener('click',()=>{
  let modalkey = q('.pizzaWindowArea').getAttribute('data-key');
